...
一人の身体を時間方向に展開して星座を作る作品。現在は、実APIから星座を取得する最初の観測画面まで実装しています。
...
星座の代表方位・高度から恒星位置は生成しません。恒星カタログ接続・身体推定・Three.js残像描画は未実装です。
fixturesはテスト専用で、実APIの失敗時にモックへ切り替えません。

## 検証

- `npm run check`：Nodeテストと本番ビルド。外部APIを呼びません。
- `npm run test:browser`：Playwrightによる画面試験。成功応答はテスト内のfixtureです。
