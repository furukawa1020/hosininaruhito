...
## 2026-09-21 実トークンによる接続成功（#34）

利用者が提供したLivlogトークンをGit管理外の.envへ設定し、指定済みの金沢市の地点で実リクエストを送信した。
トークン・座標・応答本文はファイルやログへ保存しない。
lat/lngだけではHTTP 200内にE001（パラメータエラー）。公式Markdownの必須date/hour/minを加えると成功した。
サーバー時刻をJSTの年月日・時・分へ明示変換して送信する。成功時のerrors: []も受理するよう修正。
`npm run smoke:sky` は実アプリの認証・provider・応答検証を通り、31件を取得して終了コード0。
これはライブ疎通の成功。API側タイムゾーン、現地の空との一致、実人体操作の確認は含まない。
以下の過去記録にある「トークン未設定」「lat/lngのみ」は当時の状態で、この実装では解消している。
