...
## 2026-09-21 ゲストから同じ体験へ（#37）

登録なしの開始ボタン、Firebase匿名認証＋App Checkを追加。全API・カメラ・保持/capture・Vertex計画は既存と共通で、日次quota・全体lease・緊急停止も共通。
ゲスト認証をタブ内に保持し再読み込み時は同じUIDを再利用する。映像/身体データ/同意を復元しない。通常アカウントのメモリ保持は維持。
サーバーはHCR_GUEST_ENABLEDと署名済みanonymous providerを検証し、未招待メール/custom tokenを拒否。開始待ちの停止・Esc・非表示・取消は遅い認証結果を破棄する。
専用Firebaseの匿名providerと30日後の匿名アカウント自動削除を有効化。依存追加はなく既存lockfileを維持。npm run checkは166件＋ビルド成功。画面回帰・ライブ・配備確認は後続結果を記録する。

Edgeの認証/セッション回帰37件が成功。追加したゲストの合成通し試験で撮影同意→計測→Vertex計画→全capture→終了を確認し、実人体とは区別した。モバイルのゲスト開始画面も目視確認。セルフレビューで認証処理の同時進行を防ぎ、停止後の遅いログイン破棄とゲスト設定変更時のSDK再利用を確認した。

本番Cloud Run revision `hcr-api-00003-dpc` とHosting version `eb6ced44ac7692bc` を配備。公開サイトの通常Edgeで、実匿名認証・実App Check・星をみるひとAPIの31星座取得とVertex実生成が成功した。再読み込み後の同じ匿名UID、同意の非復元、終了時の認証消去、CSP違反なしも確認。既存アカウントの実ログイン・星API31星座・ログアウト時消去も再確認した。実Vertex入力は合成契約データであり、実カメラ・実人体受入は引き続き未確認。

最初のLinux CI（35590981338）は166単体＋ビルド成功、画面81/82件。ゲスト合成通し試験がThree.jsの非同期描画開始前に仮想時計を進めていたため、既存通し試験と同じ開始状態の待機を追加した。製品コードは変更していない。最終CI結果はPR #38へ記録する。

## 2026-09-21 遊び方と画面デザインの改装（#39）

小さな英字見出し中心の画面を、日本語の大きな見出し・明るい背景・紺色の描画面へ変更。説明図で「手の光を輪に重ねて0.8秒止める」を入口から示し、星座選択→手の準備→星座づくりの3段階に整理した。接続の技術情報・計算メタデータ・任意助言は折りたたみ、同意と必要操作を表に残す。

認証・選択・カメラ・追跡・計測・配置・描画の実状態から次の操作を案内する。案内はナビゲーションのみで、撮影・送信・保持判定を自動実行しない。範囲確定後・配置後の次へ進むリンク、現在操作する箇所の強調を追加。スマートフォンでは撮影中のプレビューが計測操作と一緒に見えるようにする。

現在地は本人のボタン操作でのみ取得、小数第2位へ丸めて入力する。送信同意や検索は自動実行しない。拒否時は手入力を案内し、停止・非表示・手入力・検索・認証変更後の遅い位置取得結果を破棄する。API/認証/停止条件に変更なし。依存追加なし、lockfileを維持。

166単体＋本番ビルド成功。案内・位置取得の正常/拒否/遅い結果・PC/スマートフォンに7画面試験を追加し、既存合成通し試験に完了/停止時の案内確認を追加。画面目視で説明図の文字切れとヘッダーのリンク重なりを見つけて修正。最終回帰・配備結果は後続に記録する。実カメラ/実人体受入は#20の残件。

初回Edge全体は87/89成功。2件は試験の旧表示文字列とメタデータの移動先の追随漏れで、期待値を修正した。ゲスト通し・停止・合成カメラ・AI異常系は成功。目視で手の光が小さい点を改善し、水色14px・確定星14pxへ変更した（判定の許容幅は変更しない）。最終CIとHosting公開結果はPR #40に記録する。

## 2026-09-21 実カメラで初回人物検出が終了する問題（#41）

使用可能なUSBカメラと公開Edgeで実映像の到着を確認。人物検出は初回の遅延/見切れ/不在で停止を繰り返し、実人体の通し確認は失敗。実カメラ起動と、身体検出/完成を区別して記録した。

初回探索を最大15秒に限定して分離し、有効な新しい両手首の計測が得られるまでreach/traceへ座標を渡さない。探索中の映り方を具体的に案内し、SDKの人物候補閾値は公式既定値へ戻す。手首visibility 0.8、150ms鮮度、追跡後のロスト即停止・明示再開、保持判定は維持。映像の保存・送信は行わない。依存追加なし、lockfile維持。

実機の成功確認は修正版の公開後に参加者と再実施する。未完了の受入は#20に残し、修正の試験・レビュー・配備結果は対応PRへ記録する。

2026-09-22: 初回修正の単体171件・本番ビルド、Edgeの推定/計測/通し51件が成功。セルフレビューで初回探索中の入力時刻の逆行・重複と、画像変換待ち中の期限切れも明示的に停止するよう補強し、回帰テストを追加。最終CI・公開結果はPRへ記録する。

## 2026-09-22 画面外の手首を計測異常と混同する問題（#43）

公開#42で実カメラの有効な両手首計測（81ms）を一度確認したが、継続はframe_gap/invalid_poseで停止。実人体の通し完了は未達。有限の画面外x/yを破損値と同じinvalid_poseにして初回探索を打ち切るコード上の問題を修正する。画面外はサンプルを出さず初回だけ既存15秒内で再探索、追跡開始後は停止・明示再開を維持。非有限値/型/配列/visibility不正は即停止し、片手の見切れで反対側の破損を隠さない。座標のクランプ・閾値変更・依存追加はなくlockfile維持。実機のinvalid_poseの内訳は未取得で、修正後に切り分ける。

## 2026-09-23 一画面の体験と手の案内（#45 / #46）

標準画面をカメラ・星座の重ね合わせと、その場の案内・一つの主要操作に変更。星座/認証/位置情報は短い準備ダイアログへ移動し、全操作ページは明示的なview=detailsに残す。実際の既存入力と同意、既存ボタンのハンドラーを使い、準備/送信/撮影を勝手に実行しない。映り方の例、選んだ手で上下左右に動かす案内、3秒/30点の準備表示、実測の手首と目標から画面上の方向を表示する。表示の案内はcapture判断へ渡さない。150ms鮮度・停止・保持・実API・依存は維持し、lockfile変更なし。PC/スマートフォンの標準画面を新規E2Eで確認し、既存の詳細操作の回帰と区別する。実人体の通し完了は#20で継続。

新画面のEdge通し5件（PC/スマホ完成、ロストと再開、設定中の停止、API失敗）が成功。合成画像の目視でスマホ説明図の縮小と初回再開ラベルを修正。セルフレビューでダイアログ内にも停止して閉じる操作とゲスト終了を残し、方向案内の改行を整えた。最終CI/配布結果はPR #47へ記録する。

## 2026-09-25 天球とページ式ゲームUI（#48 / #49）

準備ダイアログを参加・場所・星座のページへ分割し、任意ログイン・説明・AI設定も独立画面へ移動。星座は左右送りと回せる立体天球で選択する。天球の点は実APIの星座代表方位/高度のみで、恒星や星座線を生成しない。描画不能時も左右選択は使える。撮影・位置送信・AIの同意と既存の停止/計測条件を保持。依存追加なし、lockfile維持。検証・公開結果は対応PRへ追記し、実人体の通し受入は引き続き#20。
